...
const Profile = () => {
  const personalInfo = {
    ...
    location: "Long Thạnh Mỹ, Thủ Đức, HCM, Vietnam",
    ...
    <div className="min-h-screen">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="relative overflow-hidden rounded-b-3xl">
          ...
          {/* Profile Section */}
          <div className="relative bg-white/95 backdrop-blur-sm mx-4 -mt-20 rounded-3xl shadow-2xl p-8 pt-32 border border-white/30">
            <div className="flex flex-col lg:flex-row items-center lg:items-start gap-8">
              ...
              <div className="flex-1 text-center lg:text-left">
                ...
                {/* Contact Info - Sắp xếp lại */}
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                  {/* Email */}
                  <a 
                    href={`mailto:${personalInfo.email}`} 
                    className="flex items-center gap-3 bg-blue-50 hover:bg-blue-100 text-blue-700 p-4 rounded-xl transition-all duration-300 hover:scale-105 shadow-sm"
                  >
                    <Mail className="w-5 h-5 flex-shrink-0" />
                    <div className="text-left min-w-0">
                      <div className="text-xs text-blue-500 uppercase tracking-wide">Email</div>
                      <div className="font-medium text-sm truncate">{personalInfo.email}</div>
                    </div>
                  </a>
                  
                  {/* Phone */}
                  <div className="flex items-center gap-3 bg-green-50 text-green-700 p-4 rounded-xl shadow-sm">
                    <Phone className="w-5 h-5 flex-shrink-0" />
                    <div className="text-left">
                      <div className="text-xs text-green-500 uppercase tracking-wide">Phone</div>
                      <div className="font-medium text-sm">{personalInfo.phone}</div>
                    </div>
                  </div>
                  ...
                  <a 
                    ...
                    className="flex items-center gap-3 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 p-4 rounded-xl transition-all duration-300 hover:scale-105 shadow-sm"
                  >
                    <Linkedin className="w-5 h-5 flex-shrink-0" />
                    <div className="text-left">
                      <div className="text-xs text-indigo-500 uppercase tracking-wide">LinkedIn</div>
                  ...
                  <a 
                    ...
                    target="_blank" 
                    rel="noopener noreferrer" 
                    className="flex items-center gap-3 bg-gray-50 hover:bg-gray-100 text-gray-700 p-4 rounded-xl transition-all duration-300 hover:scale-105 shadow-sm"
                  >
                    <Github className="w-5 h-5 flex-shrink-0" />
                    <div className="text-left">
                      <div className="text-xs text-gray-500 uppercase tracking-wide">GitHub</div>
                      <div className="font-medium text-sm">Follow</div>
                    </div>
                  </a>
                </div>
              </div>
                ...
                <div key={category} className="space-y-4">
                  ...
                  <div className="flex flex-wrap gap-3">
                    {skillList.map((skill, index) => (
                      <span 
                        key={index} 
                        className={`px-4 py-2 rounded-full text-sm font-semibold transition-all duration-300 hover:scale-110 hover:shadow-md ${getSkillColor(category)}`}
                      >
                        {skill}
                      </span>
                    ))}
